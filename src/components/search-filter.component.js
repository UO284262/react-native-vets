...
export const FilterComponent = ({
    ...
    return (
        <SearchContainer>
            {filters.forEach(f => {
                switch (f[1]) {
                    case "string": <StringFilter colection={colection} setFilterColection={setFilterColection} field={f[0]} mode={f[2]} />; break;
                    case "numeric": <NumericFilter colection={colection} setFilterColection={setFilterColection} field={f[0]} mode={f[2]} />; break;
                    case "selection": <SelectionFilter colection={colection} setFilterColection={setFilterColection} field={f[0]} options={f[2]} />; break;
                }
            })}
        </SearchContainer>
    );
...
const NumericFilter = ({
    ...
    const handleNumberChange = (number) => {
        ...
        switch (mode) {
            case "eq":
                setFilterColection(colection.filter(item =>
                    item[field] === number
                ));
                break;
            case "gt":
